...
def overloadlink_update(congestion):
    with open('../test/data/data.json') as f:
        data = json.load(f)
    for link in congestion:
        data["obj_coeffs"][link] = 10**10
    with open("../test/data/data.json", "w") as jsonFile:
...
def multipleinputtest(bwinput):
    ...
    for input in inputlist:
        ...
        with open('../test/data/bwlinklist.json') as f:
        ...
        with open("../test/data/bwlinklist.json", "w") as jsonFile:
